...
export function Depth({ market }: { market: string }) {
    ...
    useEffect(() => {
            if (scrollContainerRef.current && priceRef.current && asks && bids) {
                // FIXED: Target the inner Viewport element created by Radix UI/Shadcn
                ...
                if (viewport) {
                    ...
                    const scrollPosition =
                        priceElement.offsetTop -   // Distance from the top of the container to the price element
                        (viewport.clientHeight / 2) + // Minus half the container's visible height
                        (priceElement.clientHeight / 2); // Plus half the price element's height
    ...
    return (
        <Card className="w-full h-full bg-transparent border-0 ring-0 shadow-none">
            {/* Removed internal padding to let it fill the resizable panel */}
            <CardContent className="p-0 h-full flex flex-col"> 
                <OrderBookHeader/>
                <Total ref={scrollContainerRef}>
                    {asks && <AskTable asks={asks} />}
                    {ticker?.lastPrice && <div ref={priceRef} className="text-lg font-semibold text-center my-1 text-primary">{ticker?.lastPrice}</div>}
                    {bids && <BidTable bids={bids} />}
                </Total>
            </CardContent>
        </Card>
...
function OrderBookHeader() {
    return (
        <div className="flex justify-between text-xs text-muted-foreground px-4 py-2 border-b border-border mb-1">
            <span>Price</span>
            <span>Size</span>
            <span>Total</span>
        </div>
